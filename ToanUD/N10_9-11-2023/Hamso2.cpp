...
double f(double x) 
{
    return pow(exp(2*pow(x,2) + 1) - 2*x + 1, 2) - 5*pow(x+1, 3);
}
// Ðao ham bac 1 cua f(x)
double df(double x) 
{
    double a = exp(2*x*x+1);
	return 2*(a-2*x+1)*(4*x*a-2) - 15*(x+1)*(x+1);
}
// Ðao ham bac 2 cua f(x)
double dff(double x) 
{
    double a = exp(2*x*x+1);
	return (2*a-4*x+2)*(4*a+16*x*x*a) + 2*(4*x*a-2)*(4*x*a-2) - 30*(x+1);
}
...
int main() {
    ...
    cout <<"f2(x) = (e^(2x^2 + 1) - 2x + 1)^2 - 5*(x + 1)^3";
    cout <<"\n----------BT1----------\n";
    cout <<"Nhap gia tri x: " ;
    cin >>x;    
    cout <<"f(x)= " <<f(x) <<endl;
